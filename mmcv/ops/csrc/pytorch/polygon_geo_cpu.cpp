...
template <typename scalar_t>
Tensor polygon_iou_kernel(const Tensor& a_tsr, const Tensor& b_tsr)
{
    auto na = a_tsr.size(0);
    auto nb = b_tsr.size(0);
    Tensor iou_tsr = torch::zeros({ na, nb }, a_tsr.options().device(torch::kCPU));
...
{
    CHECK_CPU_INPUT(a_tsr);
    CHECK_CPU_INPUT(b_tsr);
    Tensor res_tsr;
